...
class Utils
{
     ...
     * @return void
     */
    public function generatrEcparam()
    {
        //椭圆曲线参数
        ...
        $this->G = $this->curve->decodePointHex('04' . $gxHex . $gyHex);
     ...
     */
    public function generateKeyPairHex()
    {
        // 随机数
        $rng        = gmp_random_bits(16);
        $d          = gmp_init(strlen(gmp_strval($this->n)), $rng);
        $d          = gmp_mod($d, gmp_sub($this->n, gmp_init(1)));
        $d          = gmp_add($d, gmp_init(1));
        $privateKey = static::leftPad(gmp_strval($d, 16), 64);

        // P = dG，p 为公钥，d 为私钥
        $P         = $this->G->multiply($d);
        $Px        = static::leftPad(gmp_strval(gmp_init($P->getX()), 16), 64);
        $Py        = static::leftPad(gmp_strval(gmp_init($P->getY()), 16), 64);
        $publicKey = '04' . $Px . $Py;
     ...
     * @return ECCurveFp
     */
    public function getGlobalCurve()
    {
        return $this->curve;
    }
    ...
    public static function parseUtf8StringToHex($input)
    {
        ...
        $words = [];
        for ($i = 0; $i < $length; $i++) {
            $words[$i >> 2] |= (static::charCodeAt($input, $i) & 0xff) << (24 - ($i % 4) * 8);
        }
     ...
     * @return string
     */
    public static function parseArrayBufferToHex($input)
    {
     ...
     * @return string
     */
    public static function leftPad($input, $num)
    {
        $length = strlen($input);
        if ($length >= $num) {
            return $input;
    ...
    public static function hexToArray($hexStr)
    {
        ...
        if ($hexStrLength % 2 !== 0) {
            $hexStr = static::leftPad($hexStr, $hexStrLength + 1);
        }
